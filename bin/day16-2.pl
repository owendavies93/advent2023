...
my @ans;
for (0..($width - 1)) {
    push @ans, count_tiles([$_, 0, 0, 1]);
    push @ans, count_tiles([$_, $height - 1, 0, -1]);
}
for (1..($height - 2)) {
    push @ans, count_tiles([0, $_, 1, 0]);
    push @ans, count_tiles([$width - 1, $_, -1, 0]);
}
